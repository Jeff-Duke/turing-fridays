...
const AdminSpike = ({ spike, createSpike, updateSpike, deleteSpike }, key ) => {
  return (
    ...
    >
      ...
      <p className="AdminSpikeAttendeees">Attendees: </p>
      <ul>
        {spike.attendees ?
          spike.attendees.map((attendee, index) => {
            return <li className="attendee" key={index}>{attendee}</li>
          })
          : <p><span className="AdminSpikeAttendees">No students have joined</span></p>
        }
      </ul>
      <p>
        Date Submitted:  <span className="AdminSpikeCreatedDate">{moment(spike.createdAt).format("MM-DD-YYYY")}</span>
      </p>
      <p>
        Spike Session Date:
